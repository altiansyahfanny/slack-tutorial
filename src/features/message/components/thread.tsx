...
import { useGetMessage } from "../api/use-get-mesagge";
import { useGenerateUploadUrl } from "@/features/upload/api/use-generate-upload-url";
import { useCreateMessage } from "../api/use-create-message";
import { useChannelId } from "@/hooks/use-channel-id";
import { toast } from "sonner";
import { useGetMessages } from "../api/use-get-mesagges";
...
const Thread = ({ messageId, onClose }: ThreadProps) => {
  ...
  const [editingId, setEditing] = useState<Id<"messages"> | null>(null);
  ...
  if (!message) {
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1">
          <ErrorFallback message="Message not found" />
        </div>
      </div>
    );
  }
